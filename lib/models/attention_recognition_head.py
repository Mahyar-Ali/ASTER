...
class AttentionRecognitionHead(nn.Module):
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    for i in range(self.max_len_labels):
      ...
      state = state.index_select(1, predecessors.squeeze().type(torch.long))
    ...
    while t >= 0:
      ...
      current_symbol = stored_emitted_symbols[t].index_select(0, t_predecessors.type(torch.long))
      t_predecessors = stored_predecessors[t].index_select(0, t_predecessors.type(torch.long)).squeeze()
    ...
    p = [step.index_select(0, re_sorted_idx.type(torch.long)).view(batch_size, beam_width, -1) for step in reversed(p)]
    p = torch.cat(p, -1)[:,0,:]
